...
### [2026-08-17] Committed dev-machine-specific paths (`/opt/homebrew/...`) in checked-in config/docs
**What happened:** `.claude/launch.json` hardcoded `runtimeExecutable` paths of `/opt/homebrew/Cellar/node/26.0.0/bin/node` and `/opt/homebrew/bin/node`, and `AGENTS.md` instructed contributors to run `PATH="/opt/homebrew/bin:$PATH" npx convex dev`. These only exist on the original author's specific MacBook (Homebrew on Apple Silicon, that exact Node Cellar version) and break for anyone else — other developers, CI, or any non-Homebrew/non-macOS environment.
**Root cause:** Absolute paths from one contributor's local shell environment were copy-pasted into checked-in project files instead of using the plain command name and relying on `PATH`/`node_modules/.bin` resolution.
**Fix applied:** Changed `launch.json` `runtimeExecutable` values to the bare `"node"` command, and removed the `PATH="/opt/homebrew/bin:$PATH"` prefix from both `npx convex dev` and `npx expo start` instructions in `AGENTS.md`. Confirmed no other committed files reference `/opt/homebrew`, `/Users/`, or other machine-specific paths (checked `.gitignore`, `.claude/settings.json`, `package.json`, `app.json`); `package-lock.json` darwin/osascript entries are legitimate cross-platform npm optional-dependency metadata and were left untouched.
**Rule going forward:** Never commit absolute paths from a personal machine (`/opt/homebrew/...`, `/Users/<name>/...`, Cellar version numbers) into project docs or `.claude/` config. Use bare command names (`node`, `npx <cmd>`) that resolve via `PATH`/`node_modules/.bin` so setup instructions work on any contributor's machine.
