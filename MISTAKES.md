...
### [2026-07-08] Playwright screenshot verification in the remote Claude Code environment needs manual path fixes
**What happened:** While building the marketing website (`website/`) and trying to visually verify pages, running a Playwright script from the scratchpad directory with `import { chromium } from 'playwright'` failed with `ERR_MODULE_NOT_FOUND` even though `npm ls -g playwright` showed it installed, and launching with `executablePath: '/opt/pw-browsers/chromium/chrome-linux/chrome'` (as suggested by the environment's own doc text) failed because the actual directory has a version suffix.
**Root cause:** Playwright is installed globally at `/opt/node22/lib/node_modules`, but Node's ESM resolver does not consult `NODE_PATH` for bare-specifier imports — the script has to physically live under a directory that has `playwright` as a resolvable dependency. Separately, the Chromium install path is versioned (`/opt/pw-browsers/chromium-1194/chrome-linux/chrome`), not the unversioned `chromium/chrome-linux/chrome` path.
**Fix applied:** Copied the screenshot script into `/opt/node22/lib/node_modules/` before running it with plain `node`, and pointed `executablePath` at the versioned `chromium-1194` directory (found via `find /opt/pw-browsers -iname '*chrome*'`).
**Rule going forward:** In this remote/cloud Claude Code environment, when no dedicated preview/screenshot tool is available and you need to drive Chromium directly with Playwright: (1) place the script inside `/opt/node22/lib/node_modules/` (or otherwise ensure it's resolvable as a dependency of a package that has `playwright` installed) rather than the scratchpad dir, and (2) always resolve the real Chromium path with `find /opt/pw-browsers -iname '*chrome*'` instead of assuming `/opt/pw-browsers/chromium/chrome-linux/chrome`.
